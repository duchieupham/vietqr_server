...
public interface TerminalBankReceiveRepository extends JpaRepository<TerminalBankReceiveEntity, Long> {
    ...
    @Query(value = "SELECT raw_terminal_code AS rawCode, type_of_qr AS qrType, id AS boxId, "
            + "terminal_code AS code, terminal_id AS terminalId FROM terminal_bank_receive "
            + "WHERE terminal_code = :terminalCode LIMIT 1", nativeQuery = true)
...
@Query(value = "SELECT b.code FROM terminal_bank_receive a "
            + "INNER JOIN terminal b on b.id = a.terminal_id "
            + "INNER JOIN merchant_member c ON (c.merchant_id = b.merchant_id AND c.terminal_id = b.id) "
            + "WHERE c.user_id = :userId AND a.bank_id IN (:bankIds) AND c.terminal_id != '' "
            + "GROUP BY b.code ", nativeQuery = true)
    List<String> getTerminalCodeByUserIdAndBankId(String userId, List<String> bankIds);
    ...
    @Query(value = "SELECT b.code FROM terminal_bank_receive a "
            ...
            + "WHERE c.user_id = :userId AND a.bank_id IN (:bankIds) AND c.terminal_id = '' "
            + "GROUP BY b.code ", nativeQuery = true)
    List<String> getTerminalCodeByUserIdAndBankIdNoTerminal(String userId, List<String> bankIds);
    ...
    @Query(value = "SELECT terminal_id AS terminalId, raw_terminal_code AS rawTerminalCode, terminal_code AS terminalCode, "
    ...
    @Query(value = "SELECT terminal_code AS terminalCode, raw_terminal_code AS rawTerminalCode FROM terminal_bank_receive "
            + "WHERE raw_terminal_code = :subRawCode LIMIT 1", nativeQuery = true)
