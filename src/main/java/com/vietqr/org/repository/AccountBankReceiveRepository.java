...
public interface AccountBankReceiveRepository extends JpaRepository<AccountBankReceiveEntity, Long> {
	...
	@Query(value = "SELECT te.id AS platformId, COALESCE(te.name, '') AS platformName, COALESCE(t.chat_id, '') AS connectionDetail "
			+ "FROM telegram_account_bank t "
			+ "JOIN telegram te ON t.telegram_id = te.id "
			+ "WHERE t.bank_id = :bankId "
			+ "UNION ALL "
			+ "SELECT le.id AS platformId, COALESCE(le.name, '') AS platformName, COALESCE(l.webhook, '') AS connectionDetail "
			+ "FROM lark_account_bank l "
			+ "JOIN lark le ON l.lark_id = le.id "
			+ "WHERE l.bank_id = :bankId "
			+ "UNION ALL "
			+ "SELECT gce.id AS platformId, COALESCE(gce.name, '') AS platformName, COALESCE(g.webhook, '') AS connectionDetail "
			+ "FROM google_chat_account_bank g "
			+ "JOIN google_chat gce ON g.google_chat_id = gce.id "
			+ "WHERE g.bank_id = :bankId "
			+ "UNION ALL "
			+ "SELECT gse.id AS platformId, COALESCE(gse.name, '') AS platformName, COALESCE(gs.webhook, '') AS connectionDetail "
			+ "FROM google_sheet_account_bank gs "
			+ "JOIN google_sheet gse ON gs.google_sheet_id = gse.id "
			+ "WHERE gs.bank_id = :bankId "
			+ "UNION ALL "
			+ "SELECT se.id AS platformId, COALESCE(se.name, '') AS platformName, COALESCE(s.webhook, '') AS connectionDetail "
			+ "FROM slack_account_bank s "
			+ "JOIN slack se ON s.slack_id = se.id "
			+ "WHERE s.bank_id = :bankId "
			+ "UNION ALL "
			+ "SELECT de.id AS platformId, COALESCE(de.name, '') AS platformName, COALESCE(d.webhook, '') AS connectionDetail "
