...
//                + "ORDER BY a.time DESC LIMIT :offset, 20"
    ...
    @Query(value = "SELECT Count(a.id) AS countTransactions "
            + "FROM transaction_receive a "
            + "LIMIT 1000", nativeQuery = true)
    CountTransactionsDTO countTransAccount();

    @Query(value = "SELECT a.id AS id, a.amount AS amount, a.bank_account AS bankAccount, "
            + "a.bank_id AS bankId, a.content AS content, "
            + "a.order_id AS orderId, a.ref_id AS refId, a.reference_number AS referenceNumber, "
            + "a.account_customer_id AS accountCustomerId "
            + "FROM transaction_receive a "
            + "WHERE a.time >= :timeCreate LIMIT 1 ", nativeQuery = true)
    TransactionReceiveSyncDTO getTransactionReceiveSyncByTime(@Param(value = "timeCreate") long time);
}
